...
/**
 * @brief Never call directly. Runs the Initial Transition when Hsm_Begin() is called. 
 * This executes the Initial State Handler function that is defined by the user. This 
 * Dispatcher will then determine the proper State Traversal and Entry Event sequence 
 * to execute.
 * 
 * @warning Within the supplied handler function, the user MUST transition into 
 * a State within their Hsm by calling @p HSM_TRAN(target_) macro. All of the
 * Hsm's states must also be initialized by calling @p HsmState_Ctor()
 * 
...
{
    bool success = false;
    HsmStatus status = (*inithndlr)(me); /* Execute Initial State Handler function. NULL check already performed in Hsm_Begin() */
    ...
    if ( (status == HSM_TRAN_STATUS || status == HSM_INTERNAL_TRAN_STATUS) )
    {
        ...
        for (HsmState * s = me->state; s != &me->top; s = s->superstate)
        {
            ...
            {
                /* User did not properly initialize this state within their Hsm. Impossible to check
                all states but this is better than nothing. */
                if (!s)     
                {
                    return false;
                }
                ...
                {
                    *(entryTrace++) = s; /* {0, State Transitioned Into, Superstate,...Substate of Top State}. This excludes Top State */
                }
            }
 ...
 * 
 * @warning Note that after the Construcor is called, me->state is still a NULL pointer.
 * @p Hsm_Begin() uses this NULL pointer as a check to see that Hsm_Begin() was not
 * previously called.
 * 
 * @param me Pointer to Hsm object.
...
{
    HsmState_Ctor(&me->top, (HsmState *)0, tophndlr);
    me->state = (HsmState *)0;
}


/**
 * @brief Executes a user-defined Initial State Handler function that
 * transitions the Hsm into a defined state. The Hsm Dispatcher will 
 * determine the proper State Traversal and Entry Event sequence to execute.
 * 
 ...
 * macro. This sets me->state equal to the State you want to transition into. 
 * If this is not followed, then the Hsm will remain in a pending 
 * (not started) state and results in Undefined Behavior when events
 * are dispatched to it. Also note that the user must use this macro 
 * instead of assigning me->state directly. 
 * 
 * @warning @p Hsm_Ctor() MUST be called beforehand. The individual states
 * of the Hsm must also be initialized by using @p HsmState_Ctor() as the 
 * Dispatcher requires this information to determine the proper State
 * Traversal sequence. 
 * 
 * @param me Pointer to Hsm object.
 * @param inithndlr The Initial State Handler function the user wants to 
 * execute when the Hsm first starts up.
 * 
 * @return True if the Hsm successfully started. This means the user supplied
 * a correctly defined Initial State Handler function and called Hsm_Ctor() 
 * before calling this routine. False otherwise.
 * 
 */
bool Hsm_Begin(Hsm * const me, const HsmStateHandler inithndlr)
{
    ...
    /**
     * Checks:
     * 1) (me):             Hsm object was initialized, ideally at compile-time.
     * 2) (me->top).hndlr:  Hsm Constructor was called previously.
     * 3) (!me->state):     Hsm_Begin wasn't already called on this Hsm Object.
     * 4) (inithndlr):      Initial State Handler function exists.
     */
    if ( (me) && ((me->top).hndlr) && (!me->state) && (inithndlr) )
    {
        success = Hsm_Init_Dispatch(me, inithndlr);
    }
    return success;
}
...
/**
 * TODO: Once done write note saying this does NOT support transitions
 * made in the Entry or Exit Events.
 * 
