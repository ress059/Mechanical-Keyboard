 ...
 * target_specific.h. USB is Little Endian so swapping macros will change based on if the
...
#else /* Big Endian. compile_checks.h will check if Endianness is defined properly in target_specific.h so we don't need an error guard here. */
