/**
 * @file rowcol_input_external_pullup.c
 * @author Ian Ress
...
#include "rowcol_input_external_pullup.h"
...
/**
 * @brief Initializes the Row/Column GPIO Pin as Hi-Z since an external Pullup Resistor is used.
 ...
 */
static void Rowcol_Input_External_Pullup_Init(const Rowcol_Input_External_Pullup * const me);


/**
 * @brief Wrapper Function that call GPIO Board Support Package to take a pin reading. Since this 
 * Row/Column is connected to an External Pullup, a keypress is registered when the pin has a 
 ...
 */
static bool Rowcol_Input_External_Pullup_Is_Key_Presed(const Rowcol_Input_External_Pullup * const me);
...
bool Rowcol_Input_External_Pullup_Ctor(Rowcol_Input_External_Pullup * const me, KB_PINSIZE_T mcu_pin_0, const Key ** const keys0, uint8_t length0)
{
     ...
     */
    static const struct Rowcol_Input_VTABLE rowcol_input_external_pullup_vtable =
    {
        .super.init         = (void (*)(const Rowcol * const me))&Rowcol_Input_External_Pullup_Init,
        .is_key_pressed     = (bool (*)(const Rowcol_Input * const me))&Rowcol_Input_External_Pullup_Is_Key_Presed
    };

    if (me != (Rowcol_Input_External_Pullup *)0) /* keys0 and length0 are already NULL-checked in Rowcol_Input Constructor */
    {
         ...
         * Override Rowcol_Init() with Rowcol_Input_External_Pullup_Init() and Rowcol_Input_Is_Key_Pressed()
         * with Rowcol_Input_External_Pullup_Is_Key_Presed(). Notice how Rowcol_Init_VCALL() now calls 
         * Rowcol_Input_External_Pullup_Init() and Rowcol_Input_Is_Key_Pressed_VCALL() calls 
         * Rowcol_Input_External_Pullup_Is_Key_Presed().
         */
        ((Rowcol *)me)->VPTR = (const struct Rowcol_VTABLE *)&rowcol_input_external_pullup_vtable;
    }
